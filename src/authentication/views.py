...
def edit_user_rh(request, user_id):
    ...
    profile_cible = getattr(user_cible, 'employeeprofile', None)

    if request.method == 'POST':
        form = RHUserBasicForm(request.POST, request.FILES, instance=profile_cible)
        if form.is_valid():
            profile = form.save(commit=False)
            # Mettre à jour le role sur User
            user_cible.role = form.cleaned_data.get('role', user_cible.role)
            user_cible.save()
            profile.save()
            messages.success(request, f"✅ Profil de {user_cible.get_full_name()} mis à jour")
            return redirect('dashboard-rh')
    else:
        # Initialiser le champ 'role' avec la valeur actuelle de User
        initial = {'role': user_cible.role}
        form = RHUserBasicForm(instance=profile_cible, initial=initial)
